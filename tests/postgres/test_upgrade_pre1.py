...
def test_0005_upgrade_preserves_enterprise_data_and_failed_ddl_rolls_back():
    engine=create_engine(URL);alembic("upgrade","head");alembic("downgrade","0005")
    ...
    with engine.connect() as db:
        assert db.scalar(text("select version_num from alembic_version"))=="0006";assert db.scalar(text("select role from users where id='u'"))=="ANALYST"
