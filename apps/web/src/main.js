...
function publicHeader() {
  return `<header class="public-nav"><div class="public-nav-inner"><a href="/" data-route class="brand">CutAIcost</a><nav aria-label="Public navigation"><a href="#product">Product</a><a href="/login" data-route>Log In</a><a href="/signup" data-route class="button">Sign Up</a><a href="/book-demo" data-route class="button primary">Book a Demo</a></nav></div></header>`;
}
async function landing() {
  ...
  ].map(([title,text])=>`<article><h3>${title}</h3><p>${text}</p></article>`).join('')}</div></div></section><section class="landing-container demo-showcase"><div><p class="eyebrow">GUIDED PRODUCT REVIEW</p><h2>See how CutAIcost fits your organization.</h2><p>Review your AI usage, spending, and optimization opportunities with our team.</p><p class="demo-proof">Tailored conversation. <span>No account required.</span></p></div><a href="/book-demo" data-route class="button primary">Book a Demo</a></section><section class="landing-container final-cta"><div><h2>Ready to analyze your own AI environment?</h2><p>Establish a private baseline and find quality-constrained opportunities.</p></div><div><a href="/book-demo" data-route class="button primary">Book a Demo</a><a href="/signup" data-route class="button">Sign Up</a></div></section></main><footer class="public-footer"><div class="landing-container">Private by design · Quality-constrained optimization · v0.2.0</div></footer>`;
}
...
function shell(content, admin = false) {
  return `<aside class="sidebar"><a href="/app/overview" data-route class="brand">AI Optimization Tool</a><nav aria-label="Application navigation">${navItems.map(([label, path]) => `<a href="${path}" data-route>${label}</a>`).join('')}${canAdmin(currentUser) ? '<a href="/admin" data-route>Administration</a>' : ''}</nav><div class="account"><a href="/app/profile" data-route>${esc(currentUser.display_name)}</a><button data-logout>Log out</button></div></aside><main id="main" class="workspace">${content}</main>`;
}
...
function capabilityRow(profile){const skills=Array.isArray(profile?.skills)?profile.skills.slice(0,4):[];if(!skills.length)return '';return `<section class="model-skills" aria-label="Model Skills and Talents"><header><strong>Model Skills &amp; Talents</strong><span class="badge">${esc(profile.rating_type)}</span></header><div class="model-skill-row" title="${esc(profile.score_explanation)}">${skills.map(skill=>`<div class="model-skill"><span>${esc(skill.skill)}</span><strong>${skill.score==null?'Not scored':`${number(skill.score)}/100`}</strong>${skill.score==null?'':`<span class="skill-bar" aria-hidden="true"><i style="width:${Math.max(0,Math.min(100,skill.score))}%"></i></span>`}<small>${esc(skill.confidence)} confidence</small></div>`).join('')}</div></section>`}
function pricingRows(items){return items.length?items.map(row=>`<tr><td>${esc(row.provider)}</td><td><details><summary>${esc(row.display_name||row.model)}</summary>${capabilityRow(row.capability_profile)}<p>${row.model_family?`Family: ${esc(row.model_family)}<br>`:''}${row.quality_tier?`Suitability tier: ${number(row.quality_tier)}<br>`:''}${Array.isArray(row.capabilities)&&row.capabilities.length?`Capabilities: ${row.capabilities.map(esc).join(', ')}<br>`:''}${row.context_window?`Context window: ${number(row.context_window)} tokens<br>`:''}Category: ${esc(row.pricing_category)}<br>Lifecycle: ${esc(row.lifecycle)}<br>Availability: ${esc(row.availability)}<br>Model source: ${esc(row.catalog_source||'Unknown')}<br>Pricing source: ${esc(row.pricing_source||'Unknown')}${row.canonical_pricing_model?`<br>Pricing inherited from canonical model: ${esc(row.canonical_pricing_model)}`:''}<br>Last pricing review: ${row.last_pricing_review?new Date(row.last_pricing_review).toLocaleDateString():'Unknown'}<br>Freshness policy: stale after ${number(row.stale_after_days)} days${row.manual_override_active?`<br>Underlying catalog: ${priceCell(row.catalog_input_price_per_1m)} input / ${priceCell(row.catalog_output_price_per_1m)} output (${esc(row.base_status)})`:''}${Object.keys(row.additional_dimensions||{}).length?`<br>Additional dimensions: ${esc(JSON.stringify(row.additional_dimensions))}`:''}</p></details></td><td>${row.pricing_category==='TEXT_REASONING'?priceCell(row.input_price_per_1m):'See details'}</td><td>${row.pricing_category==='TEXT_REASONING'?priceCell(row.cached_input_price_per_1m):'—'}</td><td>${row.pricing_category==='TEXT_REASONING'?priceCell(row.output_price_per_1m):'See details'}</td><td>${row.effective_at?new Date(row.effective_at).toLocaleDateString():'—'}</td><td><span class="badge">${esc(row.status)}</span></td></tr>`).join(''):'<tr><td colspan="7">No catalog models match these filters.</td></tr>'}
...
async function render() {
  ...
  try {
    ...
    else if (path === '/admin/pricing') app.innerHTML = await pricingPage();
