...
def bootstrap_admins(db:Session):
    ...
    for username,variable in (("Sith","ADMIN_SITH_PASSWORD"),("Beyond","ADMIN_BEYOND_PASSWORD")):
        ...
        user=User(username=username,email=f"{username.casefold()}@bootstrap.invalid",display_name=username,password_hash=hash_password(password),role="ADMIN",is_active=True,must_change_password=True)
