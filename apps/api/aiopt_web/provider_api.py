...
def public_telemetry(db:Session=Depends(db_session)):
    prices=db.scalars(select(PricingRecord).where(PricingRecord.provider=="openai",PricingRecord.effective_to.is_(None)).order_by(PricingRecord.model)).all()
    return {"provider":"openai","status":"CATALOG_AVAILABLE" if prices else "CATALOG_UNAVAILABLE","source":"CATALOG","last_refreshed_at":max((x.last_verified_at for x in prices),default=None),"latency_ms":None,"usage_available":False,"models":[{"id":x.model,"availability":"CATALOGED","pricing":{"unit":x.pricing_unit,"input":float(x.input_price),"output":float(x.output_price),"cached_input":float(x.cached_input_price) if x.cached_input_price is not None else None,"currency":x.currency,"source":x.provenance,"last_verified_at":x.last_verified_at}} for x in prices]}
