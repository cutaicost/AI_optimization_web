...
from .auth import audit,require_admin,require_csrf
...
def retention_preview(days:int|None=Query(None),_:User=Depends(require_admin),db:Session=Depends(db_session)):
...
def retention_apply(payload:RetentionIn,admin:User=Depends(require_admin),db:Session=Depends(db_session)):
...
def audit_json(_:User=Depends(require_admin),db:Session=Depends(db_session)):
...
def audit_csv(_:User=Depends(require_admin),db:Session=Depends(db_session)):
